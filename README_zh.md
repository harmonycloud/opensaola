...
如果没有显式设置 `HELM_NAMESPACE`，封装命令会先在所有命名空间查找已有的 `opensaola` release，并在原命名空间升级；找不到时才安装到 `middleware-operator`。需要固定命名空间时可设置 `n=<namespace>`，也可以继续用 `HELM_NAMESPACE=<namespace>`。
...
# Operator 配置
config:
  dataNamespace: ""                     # 留空时默认使用 Helm release 命名空间
  createDataNamespace: false            # 使用独立数据命名空间时可设为 true
