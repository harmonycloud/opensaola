...
func TestTargetUpgradeBaseline(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			middleware: &v1.Middleware{
				Spec: v1.MiddlewareSpec{Baseline: "current-baseline"},
			},
	...
	tests[0].middleware.SetAnnotations(map[string]string{v1.LabelBaseline: "target-baseline"})

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := targetUpgradeBaseline(tt.middleware); got != tt.want {
