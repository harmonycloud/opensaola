...
func NewLookupFunction(config *rest.Config) lookupFunc {
	return newLookupFunction(context.Background())
}
...
func getDynamicClientOnKind(apiversion string, kind string) (dynamic.NamespaceableResourceInterface, bool, error) {
	...
	intf, err := kubeclient.GetDynClient()
	if err != nil {
...
func getAPIResourceForGVK(gvk schema.GroupVersionKind) (metav1.APIResource, error) {
