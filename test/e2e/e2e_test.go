...
const serviceAccountName = "opensaola-controller-manager"

// metricsServiceName is the name of the metrics service of the project
const metricsServiceName = "opensaola-controller-manager-metrics-service"

// metricsRoleBindingName is the name of the RBAC that will be created to allow get the metrics data
const metricsRoleBindingName = "opensaola-metrics-binding"
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				"--clusterrole=opensaola-metrics-reader",
			...
			curlImage := os.Getenv("E2E_CURL_IMAGE")
			if curlImage == "" {
				// Default: reuse the already-built and kind-loaded manager image to avoid flaky external pulls.
				curlImage = projectImage
			}
			cmd = exec.Command("kubectl", "run", "curl-metrics", "--restart=Never",
				"--namespace", namespace,
				fmt.Sprintf("--image=%s", curlImage),
				"--overrides",
				fmt.Sprintf(`{
					"spec": {
						"containers": [{
